...
CompassDirection -> u_obj "up above"
                    ...
                    description [;
            					  if (player in mapleRoad && currentTime()) return MLookUp();
                        if (SelfPlayer.outside) return PCliffSun();
...
CompassDirection -> d_obj "ground"
                    ...
                    description [;
            					  if (player in cliff) <<Examine cliffCity>>;
                        if (player in hallwayHouse) <<Examine hallwayBasement>>;
                        "You see nothing unexpected in that direction.";
                    ];
...
! Ask:      see Answer
  ...
  Burn:     "Light my fire? Not without a lighter.";
...
!  Consult:  "You discover nothing of interest in ", (the) x1, ".";
  ...
  Jump:     "Feeling energized you, jump for a bit. Then you calm down.";
  JumpOver,Tie:
  ...
  Miscellany: switch (n) {
        ...
        42: if (x1 == 0) print "None"; else print "Only ", (number) x1;
            ...
            " available.";
        43: "Nothing to do!";
        44: "Try referring to the direct object.";
...
! Push: see Pull
  ...
  Taste:    "Do you always go around trying to taste strange things?";
  Tell: switch (n) {
